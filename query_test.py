...
def main():
    ...
    for j in rand_train:
        ...
        for i in np.arange(40)*10:
            d1,d2 = sift_feature(images[query_id],images[i])
            match_rate = match_fun(d1,d2)
            # perform folder match and skip over the particular image index: avoid 100% match
            # average the matches and find the highest ... matched folder == predicted individual
            # testing if given a database of multiple images for this person; can SIFT be used to identify them
            match_per.append(match_rate)
